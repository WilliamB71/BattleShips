...
class User(Board):
    ...
    def __init__(self, x, y, pixels, border, main_window) -> None:
        ...
        self.user_hits = []
        self.temp_ship = []
        self.ships_len = [5, 4, 3, 2, 1]
        self.orientation = Direction.SOUTH

    def temp_ship_control(self):
        if self.ships_len:
            if not self.temp_ship:
                selected_ship = self.ships_len.pop()
                self.ship_creation(selected_ship)
        if not self.ships_len and not self.temp_ship:
            return False

    def ship_creation(self, ship_len):
        if not self.temp_ship:
            self.temp_ship = [Vector2(7, 7+i) for i in range(ship_len)]
